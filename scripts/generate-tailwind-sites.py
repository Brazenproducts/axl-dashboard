...
<body class="bg-slate-950 text-slate-100 font-sans antialiased">
    ...
    <main class="max-w-4xl mx-auto px-4 py-12 pb-24">
        <div class="space-y-10">{product_cards}</div>
    </main>
        ...
        <div class="bg-slate-900 rounded-2xl shadow-xl border border-slate-800 p-6 md:p-8 flex flex-col md:flex-row gap-6 transition-all duration-300 hover:border-orange-500/40">
            ...
            <div class="flex-1 flex flex-col justify-between">
                <div>
                    ...
                    <ul class="text-slate-300 text-sm space-y-2">{bullet_items}</ul>
                </div>
                <div class="pt-4 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <a href="{affiliate_url}" target="_blank" class="w-full sm:w-auto inline-flex items-center justify-center px-8 py-4 text-base font-black rounded-xl text-gray-900 bg-amber-400 hover:bg-amber-300 shadow-md uppercase tracking-wider">Check Price</a>
                </div>
            </div>
        </div>
"""
...
portfolio_expansion_batch = {
    "besttruckbedmats.html": {
        ...
        "products": [
            {"title": "1. Dee Zee Heavy Duty Rubber Truck Bed Mat", "price": "$124.99", "url": "https://amazon.com", "bullets": "<li><strong>Premium Protection:</strong> High-density rubber.</li>"},
            {"title": "2. Husky Liners Heavy Duty Bed Xact Contour", "price": "$159.99", "url": "https://amazon.com", "bullets": "<li><strong>Custom Fit Profiles:</strong> Form-fitted design.</li>"}
        ]
    },
    "bestjeepseats.html": {
        "headline": "Top Premium Custom Jeep Wrangler Seats Ranked",
        "products": [
            {"title": "1. Corbeau Trailcat Diamond Vinyl Reclining Seats", "price": "$899.00 / Pair", "url": "https://amazon.com", "bullets": "<li><strong>Ergonomic Support:</strong> Offroad bolster styling.</li>"}
        ]
    }
}
...
print("🚀 MULTI-AGENT COMPILER ACTIVE: GENERATING PORTFOLIO FACELIFTS...")
for filename, data in portfolio_expansion_batch.items():
    ...
    for prod in data["products"]:
        cards_html += CARD_TEMPLATE.format(
            ...
            affiliate_url=prod["url"] + "?tag=brazenprodu01-20"
        )
    with open(output_dir / filename, "w", encoding="utf-8") as f:
        f.write(HTML_TEMPLATE.format(site_title=filename, headline_text=data["headline"], product_cards=cards_html))
    print(f"  └─ ✅ Automated Agent Build Success: {filename}")
print("\n🏆 SUCCESS: Complete batch configuration build completed successfully!")
